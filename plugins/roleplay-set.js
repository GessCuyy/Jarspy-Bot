...
`.trim()
  ...
  conn.reply(m.chat, `Jenis kelamin berhasil diatur sebagai *${kapital}*.`, m)
   ...
   let isNameExist = users.filter(user => user.waifu.toLowerCase() === args[1].toLowerCase()).length > 0;
...
Ketik */waifuku* untuk melihat detail waifu.`.trim(), m)}, 63000)
  ...
  } else if (args[0]?.toLowerCase() === "husbu") {
    if (user.nama == '-'  || user.gender == 'non-binary' || user.umur == '-') {
      throw `Kamu harus teregistrasi, untuk itu ketik */set*`
      return
    }
  ...
  if (!args[1] || !['shadow', 'kirigaya kazuto', 'gintoki sakata', 'itsuka shido'].includes(args[1].toLowerCase())) {
    throw `
